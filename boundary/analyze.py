...
def check_redirect_mangled(f):
    for edge in meta['edge']:
        from_sym = tuple(edge['from'])
        to_sym = tuple(edge['to'])
        if to_sym != f or from_sym[1] != to_sym[1]:
            continue
        # Might be optimized by a sched_outsider
        if from_sym in func_class['sched_outsider']:
            return True
        # Might be optimized by another optimized function
        if from_sym in func_class['mangled'] and check_redirect_mangeld(from_sym):
            return True
        # Legal to optimized by a successfully replaced function
    return False
...
if __name__ == '__main__':
    ...
    # Sanity checks
    for f in (func_class['sidecar'] | func_class['border']) & func_class['mangled']:
        assert not check_redirect_mangled(f), "trying to redirect the mangled function %s" % f[0]
